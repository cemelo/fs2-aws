...
class KinesisConsumerSpec extends FlatSpec with Matchers with BeforeAndAfterEach with Eventually {
  ...
  private abstract class WorkerContext(backpressureTimeout: FiniteDuration = 1.minute,
    ...
    val config =
      KinesisConsumerSettings("testStream", "testApp", Region.US_EAST_1, 10, 10, 10.seconds).getOrElse(throw new Error())
  ...
  private trait KinesisWorkerCheckpointContext {
    ...
    val settings =
      KinesisCheckpointSettings(maxBatchSize = Int.MaxValue, maxBatchWait = 500.millis).getOrElse(throw new Error())
